...
function ResetPassword() {
  ...
  const onSubmit = async (data: FormValues) => {
    const request = apiRequest('POST', '/api/auth/reset-password/begin', {
      json: data,
    }) as Promise<InitialData>;
    const wrapped = await errorWrappedRequest(request);
    if (!wrapped.hasError) {
      void navigate({ from: Route.fullPath, to: 'sent', viewTransition: true });
    }
  };
              ...
              <VStack gap={4} mb="4" alignItems="stretch">
                ...
                <VStack width="100%" alignItems="flex-start">
                  ...
                  <Heading as="h2" textStyle="body1">
                    If you have an existing email with us, we will send a reset password link to
                    that email.
                  </Heading>
                </VStack>
